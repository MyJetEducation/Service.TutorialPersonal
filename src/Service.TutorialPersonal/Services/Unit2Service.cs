...
namespace Service.TutorialPersonal.Services
{
	public partial class TutorialPersonalService
	{
		...
		{
			return await _taskProgressService.SetTaskProgressAsync(request.UserId, Unit2, Unit2.Tasks[1], request.IsRetry, request.Duration);
		}
		...
		public async ValueTask<TestScoreGrpcResponse> Unit2CaseAsync(PersonalTaskCaseGrpcRequest request)
		{
			...
			return await _taskProgressService.SetTaskProgressAsync(request.UserId, Unit2, Unit2.Tasks[3], request.IsRetry, request.Duration, progress);
		}
		...
		public async ValueTask<TestScoreGrpcResponse> Unit2TrueFalseAsync(PersonalTaskTrueFalseGrpcRequest request)
		{
			...
			return await _taskProgressService.SetTaskProgressAsync(request.UserId, Unit2, Unit2.Tasks[4], request.IsRetry, request.Duration, progress);
		}

		public async ValueTask<TestScoreGrpcResponse> Unit2GameAsync(PersonalTaskGameGrpcRequest request)
		{
			return await _taskProgressService.SetTaskProgressAsync(request.UserId, Unit2, Unit2.Tasks[5], request.IsRetry, request.Duration);
		}
	}
}